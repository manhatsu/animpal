    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="flex items-center gap-4 pt-4">
          {diary.image && (
            <img src={diary.image} alt="感情画像" className="w-16 h-16 rounded-full object-cover border border-neutral-700" />
          )}
          <div>
            <div className="text-sm text-neutral-400">感情: {diary.emotion}</div>
            <div className="text-xs text-neutral-500" suppressHydrationWarning>{new Date(Number(diary.id)).toLocaleString()}</div>
          </div>
        </div>
